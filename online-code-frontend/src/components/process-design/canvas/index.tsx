...
export class ProcessCanvas extends React.Component<any, any> {
  ...
  componentDidMount() {
    const id = this.props.procId;
    this.setState({ proc: { id: id } });
    this.props.onRef(this, id);
  ...
  init() {
    ...
    this.state.bpmnModeler = new CustomModeler({
      container: '#canvas_' + this.state.proc.procCode,
      //添加控制板
      propertiesPanel: {
        parent: '#js-properties-panel_' + this.state.proc.procCode
      },
      additionalModules: []
    })
    // 创建画板及图形
    ...
    EventBus.subscribe('editNode', (e: any) => {
      if (e.parent.id !== this.state.proc.procCode) {
        return;
      }
  ...
  async createNewDiagram(bpmn: string) {
    ...
    try {
      ...
      const canvas = document.getElementById('canvas_' + this.state.proc.procCode);
      if (!canvas) {
        return;
      }
      // 重绘左侧面板
      let paletteEntries: Element = canvas.getElementsByClassName("djs-palette-entries")[0];
      if (paletteEntries) {
